...
* \brief sensor/FPGA programming functions, called from  IRQ/tasklet in responce to the parameter changes
* \date  2010
*/
/*!********************************************************************************
*! FILE NAME  : pgm_functions.c
*! DESCRIPTION: sensor/FPGA programming functions, called from  IRQ/tasklet
*!              in responce to the parameter changes
...
// #define MDF2(x)
 ...
 #define ELPHEL_DEBUG_THIS 0
  ...
  int pgm_multisens     (struct sensor_t * sensor,  struct framepars_t * thispars, struct framepars_t * prevpars, int frame8);
...
/**
 * @brief initialize array of functions that program different acquisition parameters (some are sensor dependent)
 */
int init_pgm_proc(void) {
  ...
  sensorproc->pgm_func[onchange_multisens]=    &pgm_multisens;     /// chnages related to multiplexed sensors
...
int pgm_detectsensor   (struct sensor_t * sensor,  struct framepars_t * thispars, struct framepars_t * prevpars, int frame8) {
  ...
  if (frame8 >= 0) return -1; /// can only work in ASAP mode
...
///This 3 initialization commands were not here, trying to temporarily fix problem when WP was 8/16 words higher than actual data in DMA buffer
  if (GLOBALPARS(G_TEST_CTL_BITS) & (1<< G_TEST_CTL_BITS_RESET_DMA_COMPRESSOR )) {
...
//   CCAM_EXTERNALTS_DIS; /// Maybe use default as enabled?
...
/// try multisensor here (before removing MRST)   
   if (thispars->pars[P_SENSOR]==0) multisensor_pgm_detectsensor (sensor,  thispars, prevpars, frame8);  /// multisensor
   ...
   if (thispars->pars[P_SENSOR]==0) {
      mt9x001_pgm_detectsensor(sensor,  thispars, prevpars, frame8);  // try Micron 5.0 Mpixel - should return sensor type
...
/// turning on DC-DC converter may cause system to reboot because of a power spike, so start slow
   ...
   if (thispars->pars[P_SENSOR]==0) kac1310_pgm_detectsensor(sensor,  thispars, prevpars, frame8);  /// try KAC-1310 (does not exist anymore)
#endif
#ifdef CONFIG_ETRAX_ELPHEL_IBIS51300
   if (thispars->pars[P_SENSOR]==0) ibis1300_pgm_detectsensor(sensor,  thispars, prevpars, frame8); /// try IBIS5-1300 (software dead)
#endif
...
/// New - 0x80000 - reset DCM before moving phase, 0x40000 same (just to replace 0 that is no change)
//   if (phase==0) phase= 0xc0000 | (((int) sensor->sensorPhase90) << 16) | ((0x10000 + ((int)  sensor->sensorPhase)) & 0xffff); /// do not change phase if it is set already
   if (phase==0) {
     phase= 0x40000; 
...
/**
 * @brief reset and init sensor (all is done in sensor-specific functions)
...
/// @brief macros for the pgm_afterinit: update parameter, set parameter to value and update parameter, replacing zero with provided default
/*
...
/**
 * @brief restore image size, decimation,... after sensor reset or set them according to sensor capabilities if none were specified
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
...
///  if (GLOBALPARS(G_SENS_AVAIL) ) multisensor_pgm_afterinit0 (sensor, thispars, prevpars,frame8);
...
/**
 * @brief program sensor clock/phase
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP) can only work in ASAP mode
 * @return always 0
 */
...
/// Calculate and set sensor phase (only initially and when clock frequency is chnaged)
    long * cableDelay= (long *) &GLOBALPARS(G_CABLE_TIM );
    long * FPGADelay=  (long *) &GLOBALPARS(G_FPGA_TIM0 );
...
/**
 * @brief program sensor exposure - nothing to be done here, all sensor-specific
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
/**
 * @brief program i2c bytes/speed (may be per-frame?)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
 ...
 * Only validating, changing related parameters/scheduling actions, no hardware programming here
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP) (not used here)
...
//  if (GLOBALPARS(G_SENS_AVAIL) ) multisensor_pgm_window_common0 (sensor, thispars, prevpars, frame8);
 ...
 * apply both user FPS limits (both low and high), prepare data for the sensor function and sequecer
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
/**
 * @brief program analog gains (all is done in sensor-specific function)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
/**
 * @brief program sensor trigger mode (needs sensor-specific one too), programs timestamp mode (early/normal)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
//  X3X3_SEQ_SEND1(fpga_addr,  X313_WA_DCR1, X353_DCR1(EXTERNALTS, (async && thispars->pars[P_EXTERN_TIMESTAMP])?1:0));
  return 0;
}
...
/**
 * @brief program sensor input in FPGA (Bayer, 8/16 bits, FPN, test mode, number of lines)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)nff
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
 * @return OK - 0, <0 - error
 */
...
/**
 * @brief Start/single acquisition from the sensor to the FPGA (stop has latency of 1)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
/**
 * @brief stop acquisition from the sensor to the FPGA (start/single have latency of 2)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
 ...
 * is programmed after the later one (not possible because of just two tables in the FPGA - current and shadow, switching when the last table word is written
 * 
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
  ...
  for (color=0; color<4; color++) {
     ...
     if (get_locked_hash32(color)!=thispars->pars[P_GTAB_R+color]) { /// modified for this color
       *pgamma32=thispars->pars[P_GTAB_R+color];
       rslt=set_gamma_table (gamma32.hash16, gamma32.scale, NULL,  GAMMA_MODE_HARDWARE, color); /// frame8 - one ahead of the current do not lock yet
...
/**
 * @brief program histogram window (absolute from relative and window size)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
/**
 * @brief program autoexposure mode
 * TODO: Do something?
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
...
/**
 * @brief program quantization table(s)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
 * added programming coring into tables 0,1
 * @return OK - 0, <0 - error
 */
int pgm_quality    (struct sensor_t * sensor,  struct framepars_t * thispars, struct framepars_t * prevpars, int frame8) {
  MDF3(printk(" frame8=%d\n",frame8));
...
/**
 * @brief program memory channels 0 (sensor->memory) and 1 (memory->FPN)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
/**
 * @brief program memory channel 2 (memory->compressor)
...
/**
 * @brief program compressor modes (does not start/stop)
...
/**
 * @brief program focus modes (through writing the tables, so no sequencer)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP) Only ASAP is supported here
 * @return OK - 0, <0 - error
 */
...
//  struct frameparspair_t * pars_to_update[5]={
  ...
  MDF3(printk(" frame8=%d\n",frame8));
  if (frame8 >= 0) return -1; /// now can only programm in immediate mode by writing the table
...
/**
 * @brief program sequencer (int/ext)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
int pgm_trigseq    (struct sensor_t * sensor,  struct framepars_t * thispars, struct framepars_t * prevpars, int frame8) {
  ...
  MDF3(printk(" frame8=%d\n",frame8));
  if (frame8 >= PARS_FRAMES) return -1; /// wrong frame
  if (frame8 >= 0) return -1; /// ASAP only mode
...
/**
 * @brief program smart IRQ mode (needs to be on, at least bit 0)
...
/**
 * @brief recalculate sequences/latencies, according to P_SKIP, P_TRIG
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
...
/**
 * @brief restart after changing geometry  (recognizes ASAP and programs memory channel 2 then)
...
//  if (frame8 & ~PARS_FRAMES_MASK) return -1; /// wrong frame (can be only -1 or 0..7)
  if (frame8 >= PARS_FRAMES) return -1; /// wrong frame (can be only -1 or 0..7)
...
/**
 * @brief only start/stop/single (after explicitly changed, not when geometry was changed)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
 * @return OK - 0, <0 - error
 */
int pgm_compctl   (struct sensor_t * sensor,  struct framepars_t * thispars, struct framepars_t * prevpars, int frame8) {
...
/// write gamma tables (should be prepared). NOTE: now - if not yet calculated - will be now.
/// pgm_gamma should prepare table and set onchange_gammaload (in ASAP mode)
/// it is an exception for gamma tables

/**
 * @brief load gamma tables to FPGA (prepare if not yet).
 * pgm_gamma should prepare table and set onchange_gammaload (in ASAP mode)
...
///NOTE: Yes, ASAP, but - 1 frame ahead
  if (frame8 >= 0) return -1; /// only can work in ASAP mode
  int color, rslt;
  struct {
    unsigned short scale;
    unsigned short hash16;
...
/// Normally, nothing will be calculated in the next set_gamma_table() call
      rslt=set_gamma_table (gamma32.hash16, gamma32.scale, NULL,  GAMMA_MODE_HARDWARE | GAMMA_MODE_LOCK, color); /// frame8 - one ahead of the current
...
/**
 * @brief program multisensor parameters (10359 board)
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
 ...
 */
  int pgm_multisens     (struct sensor_t * sensor,  struct framepars_t * thispars, struct framepars_t * prevpars, int frame8) {
  MDF3(printk("frame8=%d\n",frame8)); /// nothing here, all in multisensor.c
  return 0;
}
...
/**
 * @brief programs FPGA registers that provide pre-scaling of the sensor data and vignetting correction
 * @param sensor    pointer to sensor static parameters (capabilities)
 * @param thispars pointer to sensor current parameters
 * @param prevpars  pointer to sensor previous parameters (not used here)
 * @param frame8    short (hardware) frame number parameters should be applied to (or "-1" - ASAP)
